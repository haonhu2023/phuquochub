...
export async function executePromotion(
  ...
  // Only ensureEvidenceArtifact()/linkEvidenceToTranslation() are called below -- both touch only
  // the first ctor arg (`repo`), confirmed by reading evidence.service.ts. The remaining
  // dependencies are genuinely unused by those two methods, so null is safe here (same pattern as
  // the PlacesService stub above); it is NOT safe to call any other EvidenceService method on this
  // instance.
