 ...
 * RIÊNG trước khi hash — KHÔNG dựa vào default encoding ngầm của `Hash.update(string)`. Xem
 * comment tương tự tại `computeApprovalEvidenceDigest()` trong `approval-evidence.contract.ts`.
 *
...
// TIMESTAMP CANONICAL (2026-08-25, Slice 0.5D1): `isValidCanonicalTimestamp()` DỜI sang
// `common/canonical-timestamp.ts` — approval-evidence.contract.ts (0.5D1) cần đúng logic canonical
// này và KHÔNG được copy-paste một bản thứ hai. Hành vi giữ NGUYÊN BYTE-FOR-BYTE; xem đầy đủ lý do
// rollover/leap-year trong file đó. `approval.approvedAt`/`source.retrievedAt` dưới đây dùng đúng
// hàm import, không đổi cách gọi.
