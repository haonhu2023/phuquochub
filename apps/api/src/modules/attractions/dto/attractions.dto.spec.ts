...
import { ListAttractionsQueryDto } from './attractions.dto';
...
function validateQuery(raw: Record<string, unknown>) {
  return validate(plainToInstance(ListAttractionsQueryDto, raw), PIPE_OPTIONS);
}

describe('ListAttractionsQueryDto', () => {
  ...
  it('page/limit: chuỗi số → number (@Type)', async () => {
    const dto = plainToInstance(ListAttractionsQueryDto, { page: '3', limit: '50' });
  ...
  it.each(['category', 'district', 'free_entry', 'family_friendly', 'indoor', 'outdoor', 'open_now'])(
