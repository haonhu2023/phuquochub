  ...
  async decide(
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      ...
      if (dto.decision === PlaceEditProposalDecision.REJECT) {
        ...
        //
        // `expected_content_version: place.content_version` (thêm 2026-09-24, tích hợp lên mainline
        // hiện tại — nhánh này viết TRƯỚC khi AddPlaceContentVersion/CAS tồn tại trên update(), nên
        // gọi update() không kèm trường này sẽ luôn 409). `place` vừa đọc dưới khoá FOR UPDATE ở
        // trên trong CÙNG transaction — content_version của nó chắc chắn còn đúng tại đây, nên CAS
        // check bên trong update() không thể trượt vì lý do "vừa đổi" giả: đây là truyền LẠI giá
        // trị vừa đọc dưới khoá, không phải một đoán mò, và hai lớp bảo vệ (base_value_hash ở trên +
        // content_version CAS bên trong update()) không mâu thuẫn — cùng bảo vệ một bất biến.
