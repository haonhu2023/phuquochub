...
// Guide CMS candidate (2026-09-18): entityId IS guide_articles.id (stable, in-place-updated row —
// same shape as RecordPlaceRevisionInput, not the pre-generated-id shape PLACE_TRANSLATION needs).
export interface RecordGuideArticleRevisionInput {
  articleId: string;
  // Snapshot = the whole article's fields + its ordered guide_blocks, serialized to jsonb.
  snapshot: object;
  diff?: Record<string, unknown> | null;
  origin: RevisionOrigin;
  changeNote?: string | null;
...
export class RevisionsService {
  ...
  // Guide CMS candidate: mirrors recordPlaceRevision() exactly — single stable entityId
  // (guide_articles.id), whole-content snapshot per save/publish. `manager` optional so the caller
  // (GuideArticlesService.saveDraft/publish) can run this INSIDE its own dataSource.transaction(),
  // same reason recordPlaceTranslationRevision() takes one.
  recordGuideArticleRevision(
    input: RecordGuideArticleRevisionInput,
    manager?: EntityManager,
  ): Promise<{ id: string; revisionNumber: number }> {
    return this.revisionsRepo.record(
      {
        entityType: RevisionEntityType.GUIDE_ARTICLE,
