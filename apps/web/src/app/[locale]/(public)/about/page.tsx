...
interface Props {
  // `string`, không phải `Locale` — type Next.js tự sinh cho segment `[locale]` luôn là `string`;
  // narrow xuống `Locale` ngay dưới (an toàn: `[locale]/layout.tsx` đã `notFound()` cho locale lạ).
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: localizedHref(locale, '/about') },
  };
}
    ...
    <LegalPage title="Giới thiệu" showUpdated={false}>
      ...
      <LegalSection heading="Cách chúng tôi xử lý dữ liệu của bạn">
        <p>
          ...
          nằm ở <Link href={localizedHref(locale, '/privacy')}>Chính sách bảo mật</Link>, và quy tắc sử dụng ở{' '}
      ...
      <LegalSection heading="Góp ý và liên hệ">
        <p>
          ...
          <Link href={localizedHref(locale, '/contact')}>trang Liên hệ</Link>.
        </p>
      </LegalSection>
    </LegalPage>
  );
