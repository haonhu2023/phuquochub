...
export function ContactsView({ placeId }: Props) {
  ...
  /**
   * `contact.version` (CAS token, 2026-09-16, sửa lại dùng xmin 2026-09-17) đọc từ CHÍNH bản ghi
   * đang sửa — luôn là giá trị đã tải khi form mở, nên 409 ở đây nghĩa đúng là "ai đó khác đã sửa
   * liên hệ này sau khi bạn mở form". `throw` (không setState) để `ContactEditor` tự hiển thị lỗi
   * NGAY DƯỚI form đang sửa, cùng khuôn `handleCreate`/`PhotosView.handleMetadataSave`.
   */
  async function handleUpdate(contact: PlaceContact, input: ContactFormInput) {
    ...
    try {
      await updatePlaceContact(contact.id, input, session.accessToken, contact.version);
    } catch (err) {
