...
import {
  canBeCover,
  formatPlacePhotoUploadedAt,
  placePhotoShortId,
  placePhotoStatusLabel,
  type PlacePhoto,
} from './types';
...
/**
 * Phần THÂN của màn hình quản lý ảnh cơ sở — tách khỏi `PhotosView` (2026-09-17) để dùng chung cho
 * CẢ HAI nơi hiển thị: trang dashboard đầy đủ (`/dashboard/places/{id}/photos`, `PhotosView` bọc
 * thêm breadcrumb/header) VÀ drawer "📷 Quản lý ảnh" ngay trên trang công khai
 * (`PlacePhotosButton`). Một nguồn logic duy nhất — không có bản sao thứ hai của luồng tải/duyệt/
 * sắp xếp/đặt bìa để hai nơi có thể lệch hành vi theo thời gian.
 *
 ...
 *
 * Ảnh chưa duyệt không có URL công khai, nên `photo.url` trỏ tới kênh nội bộ theo cơ sở
 * (`/places/{id}/media/{mediaId}/file`) — endpoint đó đứng sau `JwtAuthGuard` toàn cục (chỉ nhận
 * header `Authorization`, không có cookie dự phòng). `<img src>` KHÔNG mang được header đó, nên
 * thumbnail phải tải qua `PhotoThumbnail`/`useAuthenticatedImage` (fetch có xác thực → Object URL),
 * không gắn thẳng `photo.url` vào `<img src>` như một liên kết công khai bình thường.
 */
export function PlacePhotosManager({ placeId }: Props) {
   ...
   * thao tác, không phải banner chung ở đầu trang (cùng khuôn `ContactEditor.handleUpdate`).
   */
  ...
  return (
    <>
      ...
      {state.kind === 'ready' && state.photos.length > 0 && accessToken && (
        <>
            ...
            {state.photos.map((photo, index) => (
              <li key={photo.id} className={`${styles.tile} ${photo.is_cover ? styles.tileCover : ''}`}>
                ...
                <div className={styles.tileBody}>
                  {/* Định danh CHỐNG NHẦM LẪN — đọc trước nhãn trạng thái, luôn hiện bất kể trạng
                      thái/thao tác nào đang chạy trên tile. */}
                  <span className={styles.tileIdentifier}>
                    Ảnh #{placePhotoShortId(photo.id)} · {formatPlacePhotoUploadedAt(photo.created_at)}
                  </span>
